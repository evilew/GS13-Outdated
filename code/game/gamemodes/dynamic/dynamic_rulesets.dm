/datum/dynamic_ruleset
	...
	/// Hyper Change - Chaos Gating
	/// Events require a max or min chaos value to be called.
	var/chaos_min = 0
	var/chaos_max = 5
	/// Event requires an admin to be online
...
///Return the current threat requirement for the current population for the current rule
/datum/dynamic_ruleset/proc/get_threat(population = 0)
	pop_per_requirement = pop_per_requirement > 0 ? pop_per_requirement : mode.pop_per_requirement
	var/indice_pop = min(10,round(population/pop_per_requirement)+1)
